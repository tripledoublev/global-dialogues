...
**Purpose:** Analyzes Remesh tag data to generate unified reports on tagged responses from *Ask Opinion* questions. Combines tag categories, response text, participant sentiment, agreement scores, and demographic/segment frequencies into a comprehensive analysis.

**Prerequisites:**
1. **Tag Files:** Requires processed tag files in `Data/GD<N>/tags/` directory:
   - `all_thought_labels.csv` - Contains participant responses with assigned tags
   - `all_tag_categories.csv` - Maps tags to thematic categories
2. **Other Data Files:** Uses standardized participant and aggregate data files
3. **Preprocessing:** Run `preprocess_tag_files.py` first to generate the required tag files from raw Remesh exports

**Run Script:**
```bash
# Analyze tags for GD3:
python tools/scripts/calculate_tags.py 3
```

**How It Works:**
1. **Data Integration:** Merges tag labels with categories, participant segments, and agreement scores
2. **Metrics Calculation:** Computes response counts, average agreement, sentiment proportions per tag
3. **Segment Analysis:** Breaks down tag frequencies by demographic segments (age, location, etc.)

**Output:** Saves `tag_analysis_report.csv` to `analysis_output/GD<N>/tags/` containing:
- **Tag Information:** Question ID/Text, Category, Tag name
- **Response Metrics:** Number of responses, average/median agreement scores
- **Sentiment Analysis:** Proportion of positive/negative sentiment for each tag
- **Demographic Breakdowns:** Frequency counts by participant segments (age groups, geographic regions, etc.)
- **All (Frequency):** Total frequency count for each tag across all participants

The output provides a comprehensive view of how different themes (tags) resonate across participant segments, helping identify patterns in public opinion on AI-related topics.
